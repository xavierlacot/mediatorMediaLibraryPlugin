...
<div class="mediator_media_library_box" id="mediator_media_library_actions">
  ...
  <ul>
    <li>
      <?php
      echo link_to(__('go to the parent folder'), '@mediatorMediaLibrary?action=list&path='.$mm_media_folder_absolute_path, array('class' => 'folder'))
      ?>
    </li>
    <li>
      <?php
    ...
    <?php if (!is_null($mm_media->getType())): ?>
      <li>
        <?php
        echo cml_link_to(__('edit media'), '@mediatorMediaLibrary?action=edit&path='.$path, array('class' => 'edit'))
        ?>
      </li>
    <?php endif; ?>
    <li>
