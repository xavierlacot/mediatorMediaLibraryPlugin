...
class cleverMedia
{
  ...
  public function cache($force = false)
  {
    ...
    $this->logger->log(sprintf('{cleverMedia} caching the media "%s".', $this->filename));
  ...
  /**
   * Retrieves the associated cc_media object
   *
   * @return ccMedia
   */
  public function getCcMedia()
  {
    return ccMediaTable::retrieveByFilename($this->filename);
  }
  ...
  protected function getHandler($options = array())
  {
        ...
        || (isset($options['reload']) && $options['reload']))
    {
      ...
      {
        $handlerClass = 'cleverMedia'.ucfirst($options['type']).'Handler';
      ...
      {
        $this->handler = new cleverMediaNullHandler($this->filename, $this->filesystem);
      }

      $this->logger->log(sprintf('{cleverMedia} the media "%s" has a "%s" media handler.', $this->filename, get_class($this->handler)));
    }
  ...
  public function getType($fallback = null)
  {
    ...
    $this->logger->log(sprintf('{cleverMedia} the media "%s" has the mime-type "%s".', $this->filename, $mime_type));
  ...
  protected function initialize($type)
  {
    ...
    $this->logger->log(sprintf('{cleverMedia} the media "%s" has been initialized.', $this->filename));
  }

  /**
   * Creates the thumbnails associated to a cleverMedia
   *
  ...
  public function transform($transformation, $options)
  {
    ...
    $this->getCcMedia()->setMetadatas($this->getMetadatas());
  ...
  public function write($source)
  {
    if (preg_match('@^(?:http(s)?://)(.+)@i', $source))
    {
      ...
      {
        $tempFile = tempnam('/tmp', 'cleverMediaLibraryPlugin');
        $b = new sfWebBrowser();
