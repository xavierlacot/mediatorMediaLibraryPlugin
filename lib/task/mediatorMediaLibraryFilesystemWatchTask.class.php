<?php
class mediatorMediaLibraryFilesystemWatchTask extends sfBaseTask
{
  /**
   * @see sfTask
   */
...
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    if (!$this->filesystem->exists(mediatorMediaLibraryToolkit::getDirectoryForSize('original')))
    {
      throw new sfException(sprintf('The media library original media folder, %s, does not exists. Please initialize the library using the media:initialize task!', $media_library_root.'/'.mediatorMediaLibraryToolkit::getDirectoryForSize('original')));
    }
  ...
  protected function checkNode($node)
  {
    ...
    $path = mediatorMediaLibraryToolkit::getDirectoryForSize('original');
    ...
    foreach ($filesystem_childrens as $filesystem_children)
    {
      ...
      elseif (isset($childrens_array[$filesystem_children]))
      {
        ...
        $last_modification_date = $this->filesystem->getLastModificationDate(mediatorMediaLibraryToolkit::getDirectoryForSize('original').'/'.$child->getAbsolutePath());
      ...
      elseif (isset($files_array[$filesystem_children]))
      {
        ...
        $last_modification_date = $this->filesystem->getLastModificationDate(mediatorMediaLibraryToolkit::getDirectoryForSize('original').'/'.$file->getAbsoluteFilename());
        ...
        if ($file_modification_date < $last_modification_date)
        {
          ...
          // check right now
          $file = Doctrine::getTable('MmMedia')->find($file->getId());
          $file_modification_date = strtotime($file->getUpdatedAt());
