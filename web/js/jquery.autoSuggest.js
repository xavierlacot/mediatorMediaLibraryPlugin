...
(function($){
	$.fn.autoSuggest = function(data, options, values) {
		var defaults = {
		...
		if((d_type == "object" && d_count > 0) || d_type == "string"){
			return this.each(function(x){
				x = x+""+Math.floor(Math.random()*100000); //this ensures there will be unique IDs on the page if autoSuggest() is called multiple times
          ...
          if ('' !== val) {
      			...
      			values_input.val(values_input.val()+val+",");
          }
				}

				// initialize the values
				values_array = values.split(',');
        jQuery.each(values_array, function(i, val) {
          addValue(val);
        });

        // when submitting the form, copy back the values in the right field
        values_input.parents('form').each(function() {
          $(this).bind('submit', function() {
            input.val(values_input.val());
          });
        });
					...
					switch(e.keyCode) {
						...
						case 13: // return
							...
							if(active.length > 0){
							  ...
							  var localvalue = input.val();
							  if (localvalue.length > 0) {
    						  addValue(localvalue);
    						  input.val('');
    						  $('a.as-add', org_li).remove();
							  }
								e.preventDefault();
							}
							break;
				...
				function processData(data, query){
					...
					for(var i=0;i<d_count;i++){
						...
						if(forward){
							var formatted = $('<li class="as-result-item" id="as-result-item-'+num+'"></li>').click(function(){
									...
									if($("#as-selection-"+attr[opts.selectedItem], selections_holder).length <= 0){
										var data = data.attributes;
										input.val("").focus();
										prev = "";
										values_input.val(values_input.val()+data.value+",");
										var item = $('<li class="as-selection-item" id="as-selection-'+data[opts.selectedItem]+'"></li>').click(function(){
												opts.selectionClick.call(this, $(this));
					...
					selections_holder.removeClass("loading");
					if (matchCount <= 0){
						results_ul.html('<li class="as-message">Aucun r&eacute;sultat trouv&eacute;</li>');
					}
